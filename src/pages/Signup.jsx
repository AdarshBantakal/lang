  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '440px', background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: '40px', position: 'relative', overflow: 'hidden', boxShadow: '0 0 60px rgba(46,204,113,0.08), 0 25px 50px rgba(0,0,0,0.4)' }}>
        <div style={{ position: 'absolute', bottom: '-80px', left: '-80px', width: '200px', height: '200px', background: 'rgba(46,204,113,0.15)', borderRadius: '50%', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            ...
            <h2 style={{ fontSize: '28px', fontWeight: '800', color: '#fff', margin: '0 0 8px', letterSpacing: '-0.5px' }}>Create Account</h2>
            <p style={{ color: '#9ca3af', fontSize: '14px', margin: 0 }}>Join thousands of language learners today.</p>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '16px' }}>
              ...
              <input style={inputStyle} placeholder="Choose a username" value={name} onChange={e => setName(e.target.value)} required
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <input type="password" style={inputStyle} placeholder="Create a strong password" value={password} onChange={e => setPassword(e.target.value)} required
                onFocus={e => e.target.style.borderColor = '#2ecc71'} onBlur={e => e.target.style.borderColor = 'rgba(255,255,255,0.1)'} />
            </div>
          ...
          <p style={{ textAlign: 'center', fontSize: '14px', color: '#9ca3af', margin: 0 }}>
            ...
            <Link to="/login" style={{ color: '#2ecc71', fontWeight: '600', textDecoration: 'none' }}>Log in instead</Link>
          </p>
        </div>
      </div>
    </div>
